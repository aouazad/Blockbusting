...
iterate.contraction <- function(model, profit.function, white.valuation.function, black.valuation.function, K)
{
	...
	x.next.period.xi	<- array(0,c(K+1,2));
	xi			  		<- array(0,K+1);
	one.period.profit 	<- array(0,K+1);
	one.period.profit.try <- array(0,c(K+1,2));
	
	xi.try 		<- array(0,c(K+1,2));
	profit.try 	<- array(0,c(K+1,2));
...
valuations.solved <- solve.mpe(model,initial.broker.profit,initial.white.valuation.function,initial.black.valuation.function,K,200)
